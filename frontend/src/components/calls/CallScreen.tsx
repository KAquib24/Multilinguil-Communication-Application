...
// Complete languages list with all Indian and international languages
const LANGUAGES = [
  // Indian Languages
...
const CallScreen: React.FC = () => {
  ...
  const targetLanguageFromRedux = useSelector((state: RootState) => state.translation.targetLanguage);
const [selectedTargetLang, setSelectedTargetLang] = useState(targetLanguageFromRedux || 'en');
  ...
  const targetLanguage = useSelector((state: RootState) => state.translation.targetLanguage);
  ...
  const handleLanguageChange = (langCode: string) => {
    ...
    if (translationEnabled && activeCall) {
      socket?.emit('translation:change-language', {
        callId: activeCall.callId,
        targetLanguage: langCode
      });
      dispatch(setTargetLanguage(langCode));
      toast.success(`Translation language changed to ${langCode.toUpperCase()}`);
    }
  };
  ...
  const activeParticipants = participants?.filter((p: CallParticipant) => p.isActive) || [];
  const isGroupCall = activeParticipants.length > 2;
  const otherParticipants = participants.filter(
    (p: CallParticipant) => p.userId._id !== currentUser?._id,
  );
  const displayUser = otherParticipants.length === 1 ? otherParticipants[0].userId : null;
  ...
  const handleStartTranslationWithLang = (targetLang: string) => {
    ...
    dispatch(setTargetLanguage(targetLang));
    dispatch(toggleTranslation());
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900">
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 relative">
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <h3 className="text-2xl font-bold text-white mb-2">
                  {isGroupCall ? "Group Voice Call" : displayUser?.name || "Unknown"}
                </h3>
            ...
            <div className="absolute top-0 right-0 h-full w-80 bg-gray-900 bg-opacity-95 backdrop-blur-sm border-l border-gray-800 p-4 overflow-y-auto">
              ...
              <div className="space-y-3">
                {activeParticipants.map((participant: CallParticipant) => {
                  ...
                  return (
                    <div key={participant.userId._id} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-800">
                      ...
                      <div className="flex-1">
                        <div className="text-white font-medium">
                          {user.name}
                          {user._id === initiator._id && <span className="ml-2 text-xs text-blue-400">Host</span>}
                        </div>
                        <div className="text-xs text-gray-400">{participant.isActive ? "Active" : "Inactive"}</div>
                      </div>
                    </div>
                  );
                })}
          ...
          {translationEnabled && (
            <div className="absolute top-0 left-0 h-full w-80 bg-gray-900 bg-opacity-95 backdrop-blur-sm border-r border-gray-800 p-4 overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-lg font-semibold text-white">Live Translation</h4>
                <button onClick={handleToggleTranslation} className="text-gray-400 hover:text-white">
                  <XMarkIcon className="h-5 w-5" />
                </button>
              </div>
              <div className="mt-6">
                <h5 className="text-sm font-medium text-gray-300 mb-2">Recent Translations</h5>
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {currentTranslationSession?.segments?.slice(-5).map((segment, index) => (
                    <div key={index} className="p-2 bg-gray-800 rounded-lg">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-gray-400">{format(new Date(segment.timestamp), "HH:mm:ss")}</span>
                        <span className="text-xs text-blue-400">{Math.round(segment.confidence * 100)}%</span>
                      </div>
                      <div className="text-sm text-white mb-1">{segment.text}</div>
                      <div className="text-sm text-green-400">{segment.translatedText}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="p-6 bg-black bg-opacity-50">
          <div className="flex items-center justify-center space-x-6">
            <button onClick={handleToggleMute} className={`flex flex-col items-center ${isMuted ? "text-red-400" : "text-white"}`}>
              <div className={`h-14 w-14 rounded-full flex items-center justify-center mb-2 ${isMuted ? "bg-red-500 hover:bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}>
            ...
            {type === "video" && (
              <button onClick={handleToggleVideo} className={`flex flex-col items-center ${isVideoOff ? "text-red-400" : "text-white"}`}>
                <div className={`h-14 w-14 rounded-full flex items-center justify-center mb-2 ${isVideoOff ? "bg-red-500 hover:bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}>
                  {isVideoOff ? <VideoCameraSlashIcon className="h-6 w-6" /> : <VideoCameraIcon className="h-6 w-6" />}
                </div>
                <span className="text-xs">{isVideoOff ? "Turn On" : "Turn Off"}</span>
              </button>
            )}

            <button onClick={handleToggleTranslation} className={`flex flex-col items-center ${translationEnabled ? "text-blue-400" : "text-white"}`}>
              <div className={`h-14 w-14 rounded-full flex items-center justify-center mb-2 ${translationEnabled ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-700 hover:bg-gray-600"}`}>
                <LanguageIcon className="h-6 w-6" />
              </div>
              <span className="text-xs">{translationEnabled ? "Translation ON" : "Translate"}</span>
            </button>

            {/* Language Selector Button - Only show when translation is enabled */}
            {translationEnabled && (
              <button onClick={() => setShowLanguageSelector(true)} className="flex flex-col items-center text-white">
                <div className="h-14 w-14 rounded-full bg-purple-500 hover:bg-purple-600 flex items-center justify-center mb-2">
                  <LanguageIcon className="h-6 w-6" />
                </div>
                <span className="text-xs">{targetLanguage.toUpperCase()}</span>
              </button>
            )}

            <button onClick={handleToggleScreenShare} className={`flex flex-col items-center ${isScreenSharing ? "text-blue-400" : "text-white"}`}>
              <div className={`h-14 w-14 rounded-full flex items-center justify-center mb-2 ${isScreenSharing ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-700 hover:bg-gray-600"}`}>
                <ComputerDesktopIcon className="h-6 w-6" />
              </div>
              <span className="text-xs">{isScreenSharing ? "Stop Share" : "Share"}</span>
            </button>

            <button onClick={handleEndCall} className="flex flex-col items-center text-white">
      ...
      {/* Language Selector Modal (During Call) */}
        ...
        <div className="fixed inset-0 z-[70] bg-black bg-opacity-90 flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-2xl w-full max-w-md max-h-[80vh] overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              <h3 className="text-white text-lg font-semibold">Change Translation Language</h3>
              <button onClick={() => setShowLanguageSelector(false)} className="p-2 rounded-lg hover:bg-gray-700">
                <XMarkIcon className="h-5 w-5 text-gray-400" />
              </button>
            </div>
